...
def _circle_polygon(latitude: float, longitude: float, radius_km: float, points: int = 32) -> list[list[float]]:
	"""Create a prototype GeoJSON polygon around a zone center."""
	coordinates = []
	for index in range(points + 1):
		angle = 2 * math.pi * index / points
		lat = latitude + (radius_km / 111.0) * math.sin(angle)
		lon = longitude + (radius_km / (111.0 * max(math.cos(math.radians(latitude)), 0.1))) * math.cos(angle)
		coordinates.append([round(lon, 6), round(lat, 6)])
	return coordinates


def _exposure_geojson(zone_id: str | None = None) -> dict[str, Any]:
	datasets = store.load_all_datasets()
	zones = {zone["id"]: zone for zone in _zone_records()}
	assets = datasets["infrastructure"]
	if zone_id:
		if zone_id not in zones:
			raise KeyError(zone_id)
		assets = [asset for asset in assets if asset["zone_id"] == zone_id]
	...
	for zone in zones.values():
		...
		features.append({"type": "Feature", "geometry": {"type": "Polygon", "coordinates": [_circle_polygon(latitude, longitude, 6.0)]}, "properties": {"feature_type": "zone_boundary", "zone_id": zone["id"], "name": zone["name"], "risk_score": risk["risk_score"], "risk_level": risk["risk_level"]}})
		impact_radius = 2.5 + risk["risk_score"] * 0.065
		features.append({"type": "Feature", "geometry": {"type": "Polygon", "coordinates": [_circle_polygon(latitude, longitude, impact_radius)]}, "properties": {"feature_type": "probable_impact_area", "zone_id": zone["id"], "name": zone["name"], "radius_km": round(impact_radius, 1), "assistance": "Assistance may be needed here"}})
		source = [longitude - 0.055, latitude + 0.045]
		features.append({"type": "Feature", "geometry": {"type": "Point", "coordinates": source}, "properties": {"feature_type": "hazard_source", "zone_id": zone["id"], "name": "Potential landslide source", "description": "Steep terrain instability detected"}})
	for index, asset in enumerate(assets):
		zone = zones.get(asset["zone_id"])
		if not zone:
			continue
		latitude, longitude = zone["coordinates"]
		asset_coordinates = [round(longitude + 0.018 * ((index % 3) - 1), 6), round(latitude - 0.016 * ((index % 2) + 1), 6)]
		features.append({"type": "Feature", "geometry": {"type": "Point", "coordinates": asset_coordinates}, "properties": {**asset, "feature_type": "infrastructure", "zone_id": zone["id"]}})
...
def exposure() -> Any:
	try:
		return jsonify(_exposure_geojson(request.args.get("zone_id")))
	except KeyError:
...
def alerts() -> Any:
	zones = _zone_records()
	risks = [_risk_record(zone) for zone in zones]
	active = [build_alert(zone, risk) for zone, risk in zip(zones, risks) if risk["risk_level"] in ALERT_STATES]
	return jsonify({"alerts": active, "priority_queue": priority_queue(zones, risks), "stored": store.recent("alerts")})
...
@app.get("/api/reports")
def reports() -> Any:
	reports = store.recent("field_reports")
	zones = {zone["id"]: zone for zone in _zone_records()}
	for report in reports:
		zone = zones.get(report["zone_id"])
		if zone:
			risk_result = _risk_record(zone)
			report["current_risk"] = {"score": risk_result["risk_score"], "level": risk_result["risk_level"]}
...
def run_simulation() -> Any:
	...
	for result in results:
		zone = next(zone for zone in zones_to_simulate if zone["id"] == result["zone_id"])
		alert = build_alert(zone, result)
	...
	return jsonify({"scenario": scenario, "results": results, "alerts": alerts, "priority_queue": priority_queue(zones_to_simulate, results)})
...
def create_report() -> Any:
	...
	if payload.get("status", "Submitted") not in {"Submitted", "Under review", "Verified", "Rejected"}:
		return jsonify({"error": "invalid report status"}), 400
	try:
		_find_zone(payload["zone_id"])
	except KeyError:
		return jsonify({"error": "zone not found"}), 404
...
def update_report(report_id: int) -> Any:
	payload = request.get_json(silent=True) or {}
	status = payload.get("status")
	if status not in {"Submitted", "Under review", "Verified", "Rejected"}:
		return jsonify({"error": "invalid report status"}), 400
	if not store.update_field_report_status(report_id, status):
